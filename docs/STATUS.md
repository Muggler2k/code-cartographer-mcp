...
| `test/*.test.ts` | **352 tests passing, 0 `it.todo`** across 23 files (core map, scope/exclusion, providers + the **C# and VB Roslyn tiers** (`describe.runIf(dotnetAvailable)`), derivation, findings + **derivation v2**, the **evaluation gate**, the **benchmark gates**, analysis, viz, call-stack, **pathfinding**, **graphIndex**, **pathQueries**, **mapDiff** (pure comparator + init→edit→diff integration), the **analysisContext** seam, the **tools** table, a structural **benchmark** suite, the **dogfood seed picker** (`dogfoodSeeds`, issue #5), and the **S1 robustness corpus** (`robustness` — degrade-never-throw on pathological inputs)). Shared fixtures (`test/helpers/fixtures.ts`): `tempRepos` + `testContextMap` builders (ADR 0025). |
...
`context/sessions/2026-06-11_adr-0034-s1-summary-digest.md` and
`context/00_index/index.md`; design is recorded in ADRs 0001–0034 under
`context/07_decisions/` (ADR 0028 — the v0.2 roadmap; ADR 0033 — the Visual Basic provider tier; ADR 0034 — production validation, agents-via-Claude-Code).
