...
| `test/*.test.ts` | **305 tests passing, 0 `it.todo`** across 21 files (core map, scope/exclusion, providers + the **C# Roslyn tier** (`describe.runIf(dotnetAvailable)`), derivation, findings + **derivation v2**, the **evaluation gate**, the **benchmark gates**, analysis, viz, call-stack, **pathfinding**, **graphIndex**, **pathQueries**, **mapDiff** (pure comparator + init→edit→diff integration), the **analysisContext** seam, the **tools** table, and a structural **benchmark** suite). Shared fixtures (`test/helpers/fixtures.ts`): `tempRepos` + `testContextMap` builders (ADR 0025). |
