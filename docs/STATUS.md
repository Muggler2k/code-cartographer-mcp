...
| `test/*.test.ts` | **307 tests passing, 0 `it.todo`** across 21 files (core map, scope/exclusion, providers + the **C# Roslyn tier** (`describe.runIf(dotnetAvailable)`), derivation, findings + **derivation v2**, the **evaluation gate**, the **benchmark gates**, analysis, viz, call-stack, **pathfinding**, **graphIndex**, **pathQueries**, **mapDiff** (pure comparator + init→edit→diff integration), the **analysisContext** seam, the **tools** table, and a structural **benchmark** suite). Shared fixtures (`test/helpers/fixtures.ts`): `tempRepos` + `testContextMap` builders (ADR 0025). |
...
`context/sessions/2026-06-10_diff-mode-adr-0031.md` and
`context/00_index/index.md`; design is recorded in ADRs 0001–0031 under
`context/07_decisions/` (ADR 0028 — the v0.2 roadmap; ADR 0031 — diff/PR mode, implemented as Epic O).
