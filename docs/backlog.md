...
_Last updated: 2026-06-11 · Status: **Epics A–G implemented** (✅ complete) + **Epic H** path-finding & SQLite graph index (ADR 0023); **Epic I done** (I1–I4) — graph-traversal unification (HF-1, ADR 0024) + `find_path`/`find_callers` tools; **Epic J done** (J1–J4) — internal-seams deepening (ADR 0025); **Epic K done** (K1–K2) — findings derivation v2 + re-export visibility (ADR 0026); **Epic L done** (L1–L2) — optional C# Roslyn provider tier (ADR 0027), **deepened by ADR 0032** — host-SDK references + property/field ownership signals — **and ADR 0033** — Visual Basic joins the Roslyn sidecar (separate compilation; cross-language calls disclosed `unresolved#name`; vb-small golden fixture); **Epic M done** (M1–M2) — capability evaluation harness (ADR 0029); **Epic Q done** (Q1–Q2) — benchmark/performance gates (ADR 0030); **Epic O done** (O1–O2) — diff/PR mode `analyze_diff` (ADR 0031); **Epics N/R/S/T/P planned** per the v0.2 roadmap (ADR 0028); see per-epic notes below_
...
- ~~Deferred (ADR 0027): fields/properties as ownership signals; richer external references~~ — ✅ closed by **ADR 0032**: host-SDK TPA references (BCL-flowing internal bindings resolve; external targets stay `unresolved#name`) + property/field ownership signals (never call-graph nodes; excluded from name-collision findings via `DATA_MEMBER_KINDS`). Still deferred (ADR 0032): Tier 2 `project.assets.json` package references (decision datum recorded 2026-06-11 — Tier-1 gain on an external ASP.NET 9 repo: ZERO edge-resolution gain, unresolved ratio 69.2% unchanged, data-member signals +2385; Tier-2 upper bound ~294 edges / 10.6% of unresolved C# edges — see `csharp-roslyn-provider.md` §Measured Tier-1 gain), C# events, field-initializer edges, multi-batch resolution.
