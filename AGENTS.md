...
`npm run build` / `npm run typecheck` pass; `npm test` runs the full Vitest suite (338 tests; the C#/VB Roslyn + Roslyn-tier eval/bench suites run only where the `dotnet` CLI exists). `npm run eval` produces the capability scorecard (ADR 0029) incl. bench columns (ADR 0030).
