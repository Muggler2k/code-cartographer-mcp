// Skeleton contract for the analytical capabilities of the full planned product
// (CAS CAP-07..CAP-16). Every function throws "not implemented". All operations
...
// ---- Shared analysis substrate (Decision 0016/0019) ----
...
interface AnalysisContext {
  ...
  forward: Map<string, CallEdge[]>; // from-id -> outgoing edges
  reverse: Map<string, CallEdge[]>; // to-id -> incoming edges
  nodeById: Map<string, CallGraphNode>;
  categoryByPath: Map<string, string>;
}

/** Load the persisted map + call-graph indexes once. Returns null when not initialized. */
async function loadAnalysisContext(repositoryRoot: string): Promise<AnalysisContext | null> {
  ...
  const reverse = new Map<string, CallEdge[]>();
  const nodeById = new Map<string, CallGraphNode>();
  for (const node of map.callGraph.nodes) nodeById.set(node.id, node);
  for (const edge of map.callGraph.edges) {
    (forward.get(edge.from) ?? forward.set(edge.from, []).get(edge.from)!).push(edge);
    (reverse.get(edge.to) ?? reverse.set(edge.to, []).get(edge.to)!).push(edge);
  }
  const categoryByPath = new Map(map.files.map((f) => [f.path, f.category]));
  return { map, forward, reverse, nodeById, categoryByPath };
}
...
function resolveTargets(ctx: AnalysisContext, query: string): string[] {
  if (ctx.nodeById.has(query)) return [query];
  const nodes = ctx.map.callGraph.nodes;
  const bySymbol = nodes.filter((n) => n.symbol === query).map((n) => n.id);
  if (bySymbol.length > 0) return bySymbol;
  const byPath = nodes.filter((n) => n.path === query).map((n) => n.id);
  if (byPath.length > 0) return byPath;
  const q = query.toLowerCase();
  return nodes.filter((n) => n.symbol.toLowerCase().includes(q) || n.path.toLowerCase().includes(q)).map((n) => n.id);
}

function nodeLabel(ctx: AnalysisContext, id: string): string {
  const node = ctx.nodeById.get(id);
  return node ? `${node.symbol} (${node.path})` : id;
}

/** Traverse forward/reverse from `starts`, recording the weakest edge confidence + edge kinds seen. */
function traverse(adjacency: Map<string, CallEdge[]>, starts: string[], forward: boolean): Map<string, { conf: Confidence; kinds: Set<string> }> {
...
export async function analyzeReachability(repositoryRoot: string, target: string): Promise<ReachabilityResult> {
  ...
  if (!ctx) {
    return { analysisBoundary: BOUNDARY, subject: target, status: "unresolved", summary: "Not initialized.", reachablePaths: [], uncertainty: [INIT_UNCERTAINTY] };
  }
  const targets = resolveTargets(ctx, target);
  if (targets.length === 0) {
  ...
  const reached = traverse(ctx.forward, targets, true);
  const reachablePaths: ReachablePath[] = [...reached.entries()].map(([id, { conf, kinds }]) => {
    const grade = reachGrade(kinds, conf);
    return { id, label: nodeLabel(ctx, id), reachability: grade.reachability, confidence: grade.confidence, evidence: [`reached from ${target} via ${[...kinds].join("/")} edge(s)`] };
  });
...
export async function findDuplicateBehavior(repositoryRoot: string, subject: string): Promise<DuplicateBehaviorResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, subject, duplicatePaths: [], uncertainty: [INIT_UNCERTAINTY] };
...
export async function classifyLegacyPaths(repositoryRoot: string): Promise<LegacyClassificationResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, legacyPaths: [], uncertainty: [INIT_UNCERTAINTY] };
  return {
    analysisBoundary: BOUNDARY,
...
export async function analyzeChangeImpact(repositoryRoot: string, target: string): Promise<ChangeImpactResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, target, changeImpact: [], uncertainty: [INIT_UNCERTAINTY] };
  const targets = resolveTargets(ctx, target);
  const reached = traverse(ctx.reverse, targets, false);
  const byArea = new Map<string, { count: number; crossModule: boolean; kinds: Set<string> }>();
  const targetAreas = new Set(targets.map((id) => areaOf(ctx, ctx.nodeById.get(id)?.path ?? "")));
  for (const [id, { kinds }] of reached) {
    const path = ctx.nodeById.get(id)?.path ?? id;
...
export async function reviewChange(repositoryRoot: string, changeDescription: string): Promise<ChangeReviewResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, subject: changeDescription, alignment: "mixed", findings: [], uncertainty: [INIT_UNCERTAINTY] };
...
export async function getOwnership(repositoryRoot: string, symbolOrPath: string): Promise<OwnershipResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, subject: symbolOrPath, canonicalPaths: [], uncertainty: [INIT_UNCERTAINTY] };
...
export async function investigateFailure(repositoryRoot: string, failureReference: string): Promise<FailureInvestigationResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, subject: failureReference, hypotheses: [], requiredRuntimeConfirmation: ["Initialize the map, then investigate."], uncertainty: [INIT_UNCERTAINTY] };
  const tokens = failureReference.match(/[A-Za-z_][\w.]*/g) ?? [];
  const matched = new Map<string, CallGraphNode>();
  for (const token of tokens) {
    for (const id of resolveTargets(ctx, token)) {
      const node = ctx.nodeById.get(id);
      if (node) matched.set(id, node);
    }
  }
  const hypotheses: Finding[] = [...matched.values()].slice(0, 8).map((node) => {
    const callers = (ctx.reverse.get(node.id) ?? []).map((e) => e.from);
    const callees = (ctx.forward.get(node.id) ?? []).map((e) => e.to);
    return {
...
export async function analyzeTestPaths(repositoryRoot: string, target: string): Promise<TestPathResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, target, reachingTests: [], uncertainty: [INIT_UNCERTAINTY] };
  const targets = resolveTargets(ctx, target);
  const reached = traverse(ctx.reverse, targets, false);
  ...
  for (const [id, { conf, kinds }] of reached) {
    ...
    const grade = reachGrade(kinds, conf);
    reachingTests.push({ id, label: nodeLabel(ctx, id), reachability: grade.reachability, confidence: grade.confidence, evidence: [`test reaches ${target} via ${[...kinds].join("/")} edge(s)`] });
  }
  return {
    analysisBoundary: BOUNDARY,
    target,
...
export async function detectArchitectureDrift(repositoryRoot: string): Promise<ArchitectureDriftResult> {
  const ctx = await loadAnalysisContext(repositoryRoot);
  if (!ctx) return { analysisBoundary: BOUNDARY, driftFindings: [], uncertainty: [INIT_UNCERTAINTY] };
