...
`npm run build` and `npm run typecheck` pass. `npm test` runs the full Vitest suite (319 tests; the C#/VB Roslyn provider suite runs only where the `dotnet` CLI exists — without it C# analysis falls back to the tree-sitter tier and VB to the heuristic floor).
